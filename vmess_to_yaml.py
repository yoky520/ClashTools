...
# url-test: Use the URL, Test and select the best node.
- name: "AutoSelect"
  type: url-test
  proxies:
'''
...
def main(args):
    global head,proxy,proxy1,group_1,group_1_2,group_2,group_3,group_4,group_5,group_6,group_7,tail
    date = time.strftime('%Y%m%d')
    path = os.path.join(os.path.dirname(__file__),'%s.yml' % date)
    dict_lst = []
    ...
    for i in dict_lst:
        if i.get('network') == 'ws':
            head += proxy.format(i.get('name'),i.get('type'),i.get('server'),i.get('port'),i.get('uuid'),i.get('alterId'),i.get('cipher'),i.get('tls'),i.get('network'),i.get('ws-path'),i.get('Host'))
            proxy_name = '    - {}\n'.format(i.get('name'))
            group_1 += proxy_name
            group_2 += proxy_name
            group_5 += proxy_name
            ...
            head += proxy1.format(i.get('name'),i.get('type'),i.get('server'),i.get('port'),i.get('uuid'),i.get('alterId'),i.get('cipher'),i.get('network'))
            proxy_name = '    - {}\n'.format(i.get('name'))
            group_1 += proxy_name
            group_2 += proxy_name
            group_5 += proxy_name
